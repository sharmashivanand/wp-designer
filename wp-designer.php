...
class WPDInit {
	...
	public function __construct() {
		...
		add_action( 'template_redirect', array( $this, 'wpd_throttle_sass' ) );
	...
	public function wpd_links( $links ) {
		...
		$link = array(
			...
			'<a href="https://wordpress.org/support/plugin/wp-designer">Support</a>'
		);
	...
	public function wpd_throttle_sass() {
		...
		if( file_exists( $wpd_scss ) ) {
			...
			$wpd_format = apply_filters( 'wpd_scss_formatter', 'Leafo\\ScssPhp\\Formatter\\Expanded'  );
			
			// Leafo\\ScssPhp\\Formatter\\Compact	// Each declaration in a single line, comments are preserved
			// Leafo\\ScssPhp\\Formatter\\Compressed	// Each declaration in a single line, First comment is preserved, other comments are not preserved
			// Leafo\\ScssPhp\\Formatter\\Crunched	// Minified
			// Leafo\\ScssPhp\\Formatter\\Debug		// To debug scssphp output; outputs debug info about scssphp, not valid css
			// Leafo\\ScssPhp\\Formatter\\Expanded	// Beautified CSS
			// Leafo\\ScssPhp\\Formatter\\Nested		// Nested CSS
			...
			if(!file_exists( WPD_BASE_DIR . '/autogenerated.css' ) || filemtime( WPD_BASE_DIR . '/style.scss' ) > filemtime( WPD_BASE_DIR . '/autogenerated.css' ) ) {
				...
				$scss->setFormatter($wpd_format);

				$css .= $scss->compile( apply_filters( 'wpd_scss','@import "' . WPD_BASE_DIR . '/style.scss' . '"' ));
	...
	public function wpd_stylesheet() {
		...
		if( file_exists( $wpd_autocss ) ) {
			if(current_user_can( 'update_themes' )) {
				...
				wp_enqueue_style( 'wpd-sass' ,  WPD_BASE_URL . '/autogenerated.css', array(), filemtime( WPD_BASE_DIR . '/autogenerated.css' ) ,  'all');
            }
