	...
	public function wpd_throttle_sass() {
		...
		if ( file_exists( $wpd_scss ) ) {
			...
			if ( ! file_exists( WPD_BASE_DIR . '/autogenerated.css' ) || filemtime( WPD_BASE_DIR . '/style.scss' ) > filemtime( WPD_BASE_DIR . '/autogenerated.css' ) ) {
				...
				$css          = '@charset "UTF-8";/* Make all your changes to style.scss. This file will be overwritten by style.scss and your changes will be lost */';
				$crunched_css = $css . $scss->compileString( apply_filters( 'wpd_scss', file_get_contents( WPD_BASE_DIR . '/style.scss' ) ) )->getCss();
